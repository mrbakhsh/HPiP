    ...
    #' @param method Imputation method for missing values
    #' (mean, median or zero).
    #' @return Imputed matrix.
    #' @author Matineh Rahmatbakhsh \email{matinerb.94@gmail.com}
    #' @importFrom stats median
    #' @description Given an input matrix, impute the missing values
    ...
    impute_missing_data <-
      function(x, method = c("mean", "median", "zero")) {
        ...
        impute.f <- function(vec, method = c("mean", "median", "zero")) {
          if (missing(method)) {
            stop("Must provide at least one imputation approach")
          }
