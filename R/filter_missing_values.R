    ...
    #' @param max_miss_rate Maximal missing rate allowed for a feature;default
    #' is 20.
    #' @return A dataframe with features with missingness rate of more than
    #' user-defined threshold.
    #' @author Matineh Rahmatbakhsh, \email{matinerb.94@gmail.com}
    #' @description Given an input matrix, compute the missingness rate for
    ...
    filter_missing_values <-
      function(x, max_miss_rate = 20) {
        ...
        if (missing(max_miss_rate)) {
          stop("Must provide percentage value for features removal")
        }
        if (length(colnames(x)) == 0) {
          stop("Column (feature) names are missing...")
        }
