#' calculateGeary
#' @title Calculate Geary Autocorrelation Descriptor
...
#' \code{\link{calculateMoran}} for Moreau-Broto and
...
#' @description This function calculated normalized Geary  autocorrelation Descriptor for data input.
...
calculateGeary <-
    function(x,
        ...
        Geary_Autocor <- function(x) {
            ...
            for (i in seq_len(n.props)) {
                for (j in seq_len(nlag)) {
                    Geary[[i]][j] <- ifelse(
                        N - j > 0,
                        ((N - 1) / (2 * (N - j))) * ((sum((L[[i]][1:(N - j)] - L[[i]][(1:(N - j)) + j])^2)) / (sum((L[[i]] - Pbar[i])^2))),
                        NA
                    )
                    cGeary <- Geary
        ...
        Geaty_lists <-
            lapply(fastalist, function(x) Geary_Autocor(x[[1]]))

        # generate df from list
        tbl <- do.call(rbind, Geaty_lists)
