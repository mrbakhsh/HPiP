...
#' @param features A data frame with host-pathogen protein-protein interactions (HP-PPIs)
#' in the first column, and features to be passed to the classifier in the remaining columns.
#' @param gold_standard A data frame with gold_standard HP-PPIs and class label indicating
#' if such PPIs are positive or negative.
#' @param classifier The type of classifier to use. See \code{caret} for the
#' availbale classifiers.
#' @param resampling.method The resampling method:'boot', 'boot632', 'optimism_boot',
#' 'boot_all', 'cv', 'repeatedcv', 'LOOCV', 'LGOCV'; defaults to cv. See \code{\link[caret]{trainControl}} for more details.
#' @param ncross  Number of partitions for cross-validation;
#' defaults to 5.See \code{\link[caret]{trainControl}} for more details.
#' @param verboseIter Logical value, indicating whether to check the status of training process;
#' defaults to FALSE.
#' @param plots Logical value, indicating whether to plot the performance of ensemble learning algorithm
#' as compared to individual classifiers; defaults to TRUE.
#' If the argument set to TRUE, three plots will be generated:
#' \itemize{
#' \item{pr_curve} - precision-recall plot of ensemble classifier vs selected individual classifiers.
#' \item{roc_cruve} - ROC plot of ensemble classifier vs selected individual classifiers.
#' \item{Performance} - Plot accuracy, F1-score ,positive predictive value (PPV),sensitivity (SE),
#' and Matthews correlation coefficient (MCC) of ensemble classifier vs selected individual classifiers.
#' }
#' @return
#' Ensemble_training_output
#' \itemize{
#' \item{prediction score} - Prediction scores for whole dataset from each individual classifier.
#' \item{Best} - Selected hyper parameters.
#' \item{Parameter range} - Tested hyper parameters.
#' \item{prediction_score_test} - Scores probabilities for test data from each individual classifier.
#' \item{class_label} -  Class probabilities for test data from each individual classifier.
#' }
...
#' \item{Roc_Object} - A list of elements. See \code{\link[pROC]{roc}} for more details.
#' \item{PR_Object} - A list of elements. See \code{\link[PRROC]{pr.curve}} for more details.
#' }
#' predicted_interactions - The input data frame of pairwise interactions, including
#' classifier scores averaged across all models.
...
#' @description This function uses an ensemble of classifiers to predict interactions from the
#' sequence-based dataset. This ensemble algorithm combines different results generated from individual
...
#' gd <- gd[c(1, 5)]
#' ppi <- pred_ensembel(ind_data, gd,
#'     classifier = c("avNNet", "rf"),
...
pred_ensembel <-
    function(features,
        ...
        x <-
            ...
            filter(x == "Positive") %>%
            .$as.integer.x.
        if (x == 2) x_train$class <- factor(x_train$class, levels = rev(levels(x_train$class)))
        ...
        datScore$ensemble_score <- as.numeric(as.character(datScore$ensemble_score))
        datScore <-
            separate(datScore, PPI, c("Pathogen_protein", "Host_protein"), sep = "~", remove = FALSE)
        ## class label output
        label_output <-
            list()
        for (ml in classifier) {
        ...
        ss <- apply(ss, 2, function(x) as.factor(ifelse(x == 2, "Negative", "Positive")))
        ...
        df_perf <-
            cbind(classifier_output, ensmeble_score)
        ...
        for (i in names(df_perf)) {
            for (j in names(ss)) {
                ...
                df_perf.response[, i] <- as.factor(ifelse(df_perf.response[, i] > 0.5, "Positive", "Negative"))
                ...
                x <-
                    ...
                    filter(x == "Positive") %>%
                    .$as.integer.x.
                if (x == 2) df_perf.response[, i] <- factor(df_perf.response[, i], levels = rev(levels(df_perf.response[, i])))
                ...
                x <-
                    ...
                    filter(x == "Positive") %>%
                    .$as.integer.x.
                if (x == 2) ss1[, j] <- factor(ss1[, j], levels = rev(levels(ss1[, j])))
                cm <-
        ...
        if (plots) {
            ...
            cols <- colorspace::qualitative_hcl(nrow(pr.auc.list), palette = "Dark 3")

            plot(NA,
                ylim = c(0, 1), xlim = c(0, 1),
                main = "PR_curve",
                type = "l",
                ylab = "Percision", xlab = "Recall"
            )
            for (i in seq_len(nrow(pr.auc.list))) {
                plot(output[[i]][["PR_Object"]],
                    lwd = 2,
                    max.plot = TRUE, min.plot = TRUE, rand.plot = TRUE,
                    fill.area = TRUE, color = cols[i], add = TRUE
                )
                legend("bottomright",
                    legend = paste(pr.auc.list$model, "=", round(pr.auc.list$auc, 2)),
            ...
            print(cm_output %>%
                ...
                labs(x = "Performance metrics", y = "value", title = "", colour = "") +
            ...
            print(ggroc(Roc_Object, legacy.axes = TRUE) +
                theme(plot.title = element_text(hjust = 0.5)) +
                geom_abline(slope = 1, intercept = 0, linetype = "dashed", alpha = 0.7, color = "black") +
                coord_equal() +
                theme_bw() +
                ggtitle("ROC plots") +
                theme(plot.title = element_text(hjust = 0.5)) +
                ...
                annotation_custom(gridExtra::tableGrob(auc.list, rows = NULL, theme = mytheme),
                    xmin = unit(0.7, "npc"), xmax = unit(0.8, "npc"), ymin = 0, ymax = 0.3
                ))
        }
