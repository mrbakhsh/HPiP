...
#' FSmethod
#' @title Feature Selection via Matrix Correlation and Recursive Feature Elimination (RFE)
#' @param x A data.frame containing protein-protein interactions, class labels and
#' features.
#' @param type The feature selection type, one or two of
#' \code{filter.corr} and \code{rfeFS}.
#' @param cor.cutoff Correlation coefficient cutoff used for filtering. See \code{filter.corr}
#' for more details.
#' @param resampling.method The resampling method for RFE :'boot', 'boot632', 'optimism_boot',
#' 'boot_all', 'cv', 'repeatedcv', 'LOOCV', 'LGOCV'; defaults to cv. See \code{rfeFS} and
#' \code{\link[caret]{rfeControl}} for more details.
#' @param iter Number of partitions for cross-validation; defaults to 2. See \code{rfeFS} and
#' \code{\link[caret]{rfeControl}} for more details.
#' @param repeats For repeated k-fold cross validation only; defaults to NULL.See \code{rfeFS} and
#' \code{\link[caret]{rfeControl}} for more details.
#' @param metric  A string that specifies what summary metric will be used to
#' select the optimal feature ; default to Accuracy.See \code{rfeFS} and
#' \code{\link[caret]{rfe}} for more details.
#' @param verbose Make the output verbose.See \code{rfeFS} and
#' \code{\link[caret]{rfeControl}} for more details.

#' @return
#' If the type set to \code{filter.corr} , the output includes the following elements:
#' \itemize{
#' \item{corProfile} - A correlation matrix.
#' \item{corSelectedFeatures} - Name of features that retained after the correlation analysis.
#' \item{cordf} - A data.frame filtered.
#' }
#' If the type set to \code{rfeFS} , the output includes the following elements:
#' \itemize{
#' \item{rfProfile} - A list of elements. See \code{\link[caret]{rfe}} for more details.
#' \item{rfSelectedFeatures} - Name of features that retained in the feature selection process.
#' \item{rfdf} - A data.frame filtered.
#' }
#' If type set to \code{both} the output includes the following elements:
#' \itemize{
#' \item{rfdf} - The final data.frame that includes the selected features retained
#' after both \code{filter.corr} and \code{rfeFS} analysis.
#' }
...
#' via two approaches
#' \itemize{
#' \item \code{filter.corr} - compute matrix correlation between features and filter using a threshold.
#' \item \code{rfeFS} - perform recursive feature elimination (RFE) method wrapped with a
#' Random Forest (RF) algorithm for feature importance evaluation.
#' }
#' @export
#' @examples
#' x <- readRDS(system.file("examples/training_data.rds", package = "HPiP"))
#' x <- x[1:50, 1:10]
#' x_output <- FSmethod(x, type = "both", cor.cutoff = 0.7)
FSmethod <- function(x, type = c("cor", "rfe", "both"),
    ...
    if (missing(type)) stop("Must provide at least one feature selection approach")
