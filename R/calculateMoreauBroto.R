#' calculateMoreauBroto
#' @title Calculate Normalized Moreau-Broto Autocorrelation Descriptor
...
#' @seealso See \code{\link{calculateMoran}} and
#' \code{\link{calculateGeary}} for Moran and
...
#' @description This function calculated normalized Moreau-Broto  autocorrelation Descriptor for data input.
...
calculateMoreauBroto <-
    function(x,
        ...
        # Compute Moreau-Broto Autocorrelation Descriptor
        MB_Autocor <- function(x) {
            ...
            for (i in seq_len(n.props)) {
                for (j in seq_len(nlag)) {
                    MB[[i]][j] <- ifelse(
                        N - j > 0,
                        sum(L[[i]][1:(N - j)] * L[[i]][(1:(N - j)) + j]) / (N - j), NA
                    )
                    cMB <- MB
                    cMB <- unlist(cMB)
                }
        ...
        MB_lists <-
            lapply(fastalist, function(x) MB_Autocor(x[[1]]))

        # generate df from list
        tbl <- do.call(rbind, MB_lists)
