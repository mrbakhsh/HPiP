...
output: 
    ...
    highlight: "textmate"
vignette: >
  %\VignetteIndexEntry{HPiP overview}
...
```{r, results='hide'}
# Convert the list of seqeunces obtained in the previous section to data.frame 
...
df1[is.na(df1)] <- ""
df1 %>%
  kbl(caption = "Amino acid attributes and the division of amino acid into 
      three-group") %>%
...
df2[is.na(df2)] <- ""
df2 %>%
  kbl(caption = "Composition of k-spaced amino acid pairs.Given 400 (20 × 20) 
      amino acid pairs and four values for k (k = 1–4), there are 1600 
      attributes generated for the KSAAP feature.",col.names = NULL) %>%
...
[SummerizedExperiment](https://bioconductor.org/packages/3.13/bioc/html/SummarizedExperiment.html)
...
(`svmRadial`), neural networks using model averaging (`avNNET`), and random 
...
analysis, using a relaxed classifier confidence ensemble score cutoff of 0.55.
```{r}
pred_interactions <- filter(pred_interactions, ensemble > 0.55)
...
<img src="Figures/ROC_Curve.png" width="100%" 
...
<img src="Figures/pr-curve.png" width="100%" 
class="img-responsive" align="center">
...
<img src="Figures/point_plot.png" width="100%" 
class="img-responsive" align="center">
...
SARS-CoV2-E-human, we can run the following command:
